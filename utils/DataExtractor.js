...
class DataExtractor {
   ...
   * @param {number} frameId - 대상 프레임 ID (Optional, default: 0)
   * @returns {Promise<Object>} 응답 객체
   */
  async sendMessageToContentScript(message, frameId = 0) {
    try {
      ...
      console.error(`❌ Content Script 통신 실패 (Frame: ${frameId}):`, error);
      throw error;
    }
  }
   ...
   * @param {number} frameId - 대상 프레임 ID (Optional)
   * @returns {Promise<void>}
   */
  async extract(type, selector, targetElement, onSuccess = null, frameId = 0) {
    if (!selector) {
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(`${type} 추출 응답 (Frame: ${frameId}):`, response);
   ...
   * @returns {Promise<void>}
   */
  async extractAssets(selector, targetElement, onSuccess = null, frameId = 0) {
    return this.extract('assets', selector, targetElement, onSuccess, frameId);
  }
   ...
   * @returns {Promise<void>}
   */
  async extractPrice(selector, targetElement, onSuccess = null, frameId = 0) {
    return this.extract('price', selector, targetElement, onSuccess, frameId);
  }
}
